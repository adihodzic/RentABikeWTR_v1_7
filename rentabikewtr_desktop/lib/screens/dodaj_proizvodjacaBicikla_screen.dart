...
class DodajProizvodjacaBiciklaScreen extends StatefulWidget {
  DodajProizvodjacaBiciklaScreen({super.key});

  @override
  State<DodajProizvodjacaBiciklaScreen> createState() =>
      _DodajProizvodjacaBiciklaScreenState();
}

class _DodajProizvodjacaBiciklaScreenState
    extends State<DodajProizvodjacaBiciklaScreen> {
  TextEditingController _nazivProizvodjacaController = TextEditingController();

  ProizvodjaciBicikla?
      proizvodjacBicikla; //-- ako koristimo prosljeđivanje objekta

  ProizvodjaciBiciklaProvider? _proizvodjaciBiciklaProvider = null;
  ProizvodjaciBiciklaPregledProvider? _proizvodjaciBiciklaPregledProvider =
      null;

  List<ProizvodjaciBiciklaPregled> proizvodjaciBiciklaPregled = [];
  ...
  void initState() {
    ...
    _proizvodjaciBiciklaProvider = context.read<ProizvodjaciBiciklaProvider>();
    _proizvodjaciBiciklaPregledProvider =
        context.read<ProizvodjaciBiciklaPregledProvider>();

    proizvodjacBicikla = ProizvodjaciBicikla(nazivProizvodjaca: null);
  ...
  Future loadData() async {
    var tmpProizvodjaciBiciklaPregled =
        await _proizvodjaciBiciklaPregledProvider?.get();

    setState(() {
      proizvodjaciBiciklaPregled = tmpProizvodjaciBiciklaPregled!;
    });
  }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text(
                            "RentABikeWTR -Dodaj proizvodjaca bicikla",
                            style: TextStyle(
                                fontSize: 30,
  ...
  void _updateModeliBiciklaData() {
    setState(() {
      ...
      if (!isPostojeciNaziv(_nazivProizvodjacaController.text)) {
        proizvodjacBicikla!.nazivProizvodjaca =
            _nazivProizvodjacaController.text;
      } else {
        dupliNaziv = true;
        _nazivProizvodjacaController.text = "";
  ...
  bool isPostojeciNaziv(String input) {
    var brojac = 0;
    for (var tip in proizvodjaciBiciklaPregled) {
      if (input == tip.nazivProizvodjaca) {
        brojac = brojac + 1;
      }
    }
    if (brojac > 0) {
